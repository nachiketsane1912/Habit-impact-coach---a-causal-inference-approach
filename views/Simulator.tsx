    ...
    <div className="flex flex-col h-[calc(100vh-2rem)] bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
      ...
      <div className="bg-white border-b border-slate-100 p-6 flex justify-between items-center z-10">
        <div>
          ...
          <p className="text-sm text-slate-500">Simulate outcomes based on your historical causal model.</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-slate-200' : 'bg-purple-100 text-purple-600'}`}>
      ...
      <div className="p-4 bg-white border-t border-slate-100">
           ...
           <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
             {suggestions.map((s, i) => (
               <button 
                key={i} 
                onClick={() => setQuery(s)}
                className="whitespace-nowrap px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-full text-xs font-medium text-slate-600 transition-colors"
               >
                 {s}
               </button>
             ))}
        ...
        <form onSubmit={handleSend} className="relative">
          <input
            ...
            className="w-full bg-slate-100 text-slate-800 placeholder-slate-400 rounded-xl py-4 pl-6 pr-14 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all"
          />
          <button 
            type="submit" 
            disabled={!query.trim() || isTyping}
            className="absolute right-2 top-2 p-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            <Send size={20} />
          </button>
        </form>
