...
import { 
  ...
  AreaChart, Area, ReferenceLine, ComposedChart, Legend
} from 'recharts';
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">Your Health Impact</h2>
          <p className="text-sm md:text-base text-slate-500 mt-1">Daily correlations and causal drivers.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {/* Driver 1 */}
        <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-2xl p-5 md:p-6 text-white shadow-lg shadow-indigo-900/20 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 text-indigo-200 mb-2 text-xs font-semibold uppercase tracking-wider">
              <Activity size={14} /> Top Driver #1
            </div>
            <h3 className="text-xl md:text-2xl font-bold mb-1">Caffeine Timing</h3>
            <div className="flex items-center gap-2 mt-4">
               <div className="bg-white/20 p-2 rounded-lg">
                 <ArrowDownRight size={20} className="text-red-300" />
               </div>
               <div>
                 <p className="text-xs md:text-sm text-indigo-100">Negative Impact on</p>
                 <p className="font-bold text-sm md:text-base">Sleep Quality</p>
               </div>
            </div>
        ...
        {/* Driver 2 */}
        <div className="bg-gradient-to-br from-teal-600 to-teal-800 rounded-2xl p-5 md:p-6 text-white shadow-lg shadow-teal-900/20 relative overflow-hidden group">
           <div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-10 rounded-full blur-2xl transform translate-x-10 -translate-y-10 transition-transform group-hover:scale-110"></div>
           <div className="relative z-10">
            <div className="flex items-center gap-2 text-teal-200 mb-2 text-xs font-semibold uppercase tracking-wider">
              <Activity size={14} /> Top Driver #2
            </div>
            <h3 className="text-xl md:text-2xl font-bold mb-1">Morning Cardio</h3>
            <div className="flex items-center gap-2 mt-4">
               <div className="bg-white/20 p-2 rounded-lg">
                 <ArrowUpRight size={20} className="text-green-300" />
               </div>
               <div>
                 <p className="text-xs md:text-sm text-teal-100">Positive Impact on</p>
                 <p className="font-bold text-sm md:text-base">Energy Level</p>
               </div>
            </div>
        ...
        {/* Placeholder */}
        <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 shadow-sm flex flex-col justify-center items-center text-center">
           <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center text-slate-400 mb-3">
             <Activity size={24} />
           </div>
           <p className="text-slate-500 font-medium text-sm md:text-base">Looking for more drivers...</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100">
           <div className="mb-4 md:mb-6">
             ...
             <h3 className="text-lg md:text-xl font-bold text-slate-800">Your "Response Profile"</h3>
           ...
           <div className="h-56 md:h-64 relative">
             ...
             <div className="absolute bottom-4 left-0 w-full text-center text-[10px] md:text-xs font-bold text-slate-400 px-4 flex justify-between">
                <span>LOW RESPONDER</span>
                <span>HIGH RESPONDER</span>
             </div>
           </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="mb-4 md:mb-6">
             ...
             <h3 className="text-lg md:text-xl font-bold text-slate-800">Magnesium Supplement Impact</h3>
             <p className="text-sm text-slate-500 mt-1">
               Comparing your actual sleep (solid) vs. the counterfactual "what if" (dashed).
             </p>
           </div>
           
           <div className="h-56 md:h-64">
             <ResponsiveContainer width="100%" height="100%">
               <ComposedChart data={interventionData} margin={{top: 10, right: 10, left: -20, bottom: 0}}>
                 ...
                 <ReferenceLine x="Day 10" stroke="#ef4444" strokeDasharray="3 3" label={{ value: 'Started', position: 'insideTopLeft', fill: '#ef4444', fontSize: 10 }} />
                 
                 <Line type="monotone" dataKey="actual" name="Actual" stroke="#4f46e5" strokeWidth={3} dot={false} />
                 <Line type="monotone" dataKey="counterfactual" name="Counterfactual" stroke="#94a3b8" strokeDasharray="5 5" strokeWidth={2} dot={false} />
               </ComposedChart>
             </ResponsiveContainer>
           </div>
      ...
      <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div className="h-64 md:h-72">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={recentLogs} margin={{top: 5, right: 0, left: -20, bottom: 0}}>
