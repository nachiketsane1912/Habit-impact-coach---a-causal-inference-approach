      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl font-bold">Causal Inference Engine</h1>
          </div>
          ...
          {!insights && !isLoading && (
            <button 
              ...
              className="bg-teal-500 hover:bg-teal-400 text-slate-900 font-bold py-3 px-8 rounded-full transition-all flex items-center gap-2 shadow-lg shadow-teal-500/25"
            >
              Run Causal Model
              <ArrowRight size={18} />
            </button>
          ...
          {isLoading && (
            <div className="flex items-center gap-3 text-teal-300">
              ...
              <span>Analyzing {logs.length} data points with Counterfactual Logic...</span>
            </div>
          )}
          
          {error && <p className="text-red-400 mt-4 bg-red-900/20 p-3 rounded-lg border border-red-500/30">{error}</p>}
        </div>
      </div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h3 className="text-xl font-bold text-slate-800">{insight.factor}</h3>
                  </div>
